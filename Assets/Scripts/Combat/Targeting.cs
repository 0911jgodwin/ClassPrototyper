...
public class Targeting : MonoBehaviour
{
    ...
    Health targetHealth;
    public Transform targetDebuffs;
    public DebuffIndicator debuffPrefab;
    private EffectManager targetEffects;
    ...
    private void SelectTarget(GameObject newTarget)
    {
        ...
        targetEffects = currentTarget.GetComponent<EffectManager>();
    ...
    private void GetDebuffs()
    {
        List<TimedEffect> effectList = targetEffects.GetDebuffs();
        foreach (var effect in effectList)
        {
            DebuffIndicator debuff = Instantiate(debuffPrefab, targetDebuffs);
            debuff.castIcon.sprite = effect.sprite;
            debuff.duration.text = Mathf.Round(effect.duration).ToString();
            debuff.stackCount.text = effect.effectStacks.ToString();
        }
    }

    private void ClearDebuffs()
    {
        foreach (Transform child in targetDebuffs)
        {
